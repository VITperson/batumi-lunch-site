...
export default function PlannerScreen({ searchParams }: PageProps) {
  ...
  const checkoutMutation = useMutation<PlannerCheckoutResponse, unknown, PlannerCheckoutRequest>({
    mutationFn: (body) => {
      if (!accessToken) {
        return Promise.reject(new Error("Чтобы оформить заказ, войдите в аккаунт."));
      }
      return checkoutPlannerOrder(accessToken, body);
    },
    onSuccess: (data) => {
      setCheckoutError(null);
      setCheckoutNotice(
        `Заказ сохранён! Номер шаблона: ${data.templateId}. Мы свяжемся для подтверждения деталей.`,
      );
    },
    onError: (error) => {
      if (isAxiosError(error)) {
        const data = error.response?.data as { message?: string; detail?: string; field?: string } | undefined;
        const detail = data?.message || data?.detail;
        setCheckoutError(detail || "Не удалось оформить заказ. Попробуйте позже.");
        return;
      }
      ...
      setCheckoutError("Не удалось оформить заказ. Попробуйте позже.");
    },
  });
  ...
  const handleCheckout = useCallback(() => {
    if (!hasActiveSelection || checkoutMutation.isPending) {
      return;
    }

    const trimmedAddress = plannerState.address.trim();
    if (!trimmedAddress) {
      setCheckoutError("Укажите адрес доставки, чтобы оформить заказ.");
      return;
    }
    if (!accessToken) {
      setCheckoutError("Чтобы оформить заказ, войдите в аккаунт или зарегистрируйтесь.");
      return;
    }
    ...
    const promoCode = plannerState.promoCode.trim();
    const payload: PlannerCheckoutRequest = {
      address: trimmedAddress,
      promoCode: promoCode.length > 0 ? promoCode : undefined,
      repeatWeeks: plannerState.repeatWeeks,
      weeksCount: clampNumber(plannerState.weeksCount, 1, MAX_WEEKS),
      selections: calcPayload.selections,
      weeks: weeksPayload,
    };
    ...
    hasActiveSelection,
    checkoutMutation,
    plannerState.address,
    accessToken,
    plannerState.promoCode,
    plannerState.repeatWeeks,
    plannerState.weeksCount,
    calcPayload,
    weeksPayload,
  ]);
...
function PlannerSummary({
  ...
  const buttonDisabled = disabled || isCalculating || isSubmitting;
  const checkoutButtonLabel = isCalculating
    ? "Пересчитываем…"
    : isSubmitting
      ? "Оформляем заказ…"
      : `Оформить ${totalPortions} порций за ${formatCurrency(total, currency)}`;
  ...
  return (
    <aside className="self-start rounded-3xl border border-slate-200/70 bg-white/90 p-6 shadow-lg shadow-[rgba(27,27,27,0.05)] lg:sticky lg:top-24">
      ...
      {checkoutNotice && (
        <div className="mt-4 flex items-start justify-between gap-3 rounded-2xl bg-emerald-50 px-4 py-3 text-xs text-emerald-700">
          <p className="flex-1">{checkoutNotice}</p>
          {onDismissCheckoutMessage && (
            <button
              type="button"
              onClick={onDismissCheckoutMessage}
      ...
      {checkoutError && (
        <div className="mt-4 flex items-start justify-between gap-3 rounded-2xl bg-rose-50 px-4 py-3 text-xs text-rose-700">
